...
export async function register(req, res) {
  try {
    // augment body with ip/device information
    const payload = {
      ...req.body,
      ipAddress: req.ip || req.connection.remoteAddress,
      deviceId: req.body.deviceId || null
    };
    const { user, tokens } = await AuthService.registerUser(payload);
...
export async function login(req, res) {
  try {
    ...
    try {
      ...
      console.warn('Failed to log login attempt:', e.message);
    }
    res.status(400).json({ message: typeof error.message === 'string' ? error.message : 'An error occurred' });
  }
}
...
export async function adminLogin(req, res) {
  try {
    ...
    // record admin login success as well
    await AuthService.recordLoginAttempt({
      phone: req.body.phone,
      userId: user._id,
      ip: req.ip || req.connection.remoteAddress,
    ...
    try {
      ...
      console.warn('Failed to log admin login attempt:', e.message);
    }
  ...
  try {
    ...
    res.json({
      user: {
        ...
        classGrade: user.class === '10' ? '10th' : user.class === '12' ? '12th' : 'Other',
        profileCompleted: user.profileCompleted,
        // Referral / rewards / access info for frontend display
        referralCode: user.referralCode,
        referralStats: user.referralStats,
        freeQuizCredits: user.freeQuizCredits,
        hasPdfAccess: user.hasPdfAccess,
        temporaryPdfCredits: user.temporaryPdfCredits
      }
    });
  } catch (error) {
