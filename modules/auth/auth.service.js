...
export async function registerUser({ phone, email, otp, password, name, age, gender, schoolName, classGrade, referralCode, deviceId, ipAddress }) {
  // Normalize phone number
  let normalizedPhone = phone;
  if (phone) {
    normalizedPhone = phone.replace(/[\+\s-]/g, '');
  ...
  if (existing) {
    ...
    existing.lastLoginIp = ipAddress || existing.lastLoginIp;
  ...
  // Handle referral code: create PENDING referral record
  if (referralCode && referralCode.trim()) {
    try {
      ...
      // Create PENDING referral record (will be completed when user makes first payment)
      ...
      // update referrer stats counters
      ...
      // Log but don't break registration on referral errors
      console.error("Referral processing error during registration:", err.message);
      // Remove referredBy if there was an error
  ...
  // generate a referral code for the new user if not already present
  try {
    const { generateReferralCode } = await import('../user/streak.service.js');
    if (!user.referralCode) {
      await generateReferralCode(user._id);
    }
  } catch (err) {
    console.warn('Referral code generation failed during registration:', err.message);
  }
...
export async function loginUser({ phone, password, ipAddress, deviceId }) {
  // Only allow login with phone number
  if (!phone) throw new Error("Phone number is required for login");
  if (!password) throw new Error("Password is required");

  // simple stripped version for logging purposes
  const logPhone = phone.replace(/[^\d]/g, '');

  // allow optional country code prefix 91, but ultimately we want 10 local digits
  ...
  // if it is already 12 digits and starts with 91 we leave as-is

  const user = await User.findOne({ phone: normalizedPhone });
  if (!user) throw new Error("Invalid credentials");

  // block login if user is flagged as blocked
  ...
  // Update lastLoginAt and optionally lastLoginIp/deviceId using an atomic update
  const updateFields = { lastLoginAt: new Date() };
  if (ipAddress) updateFields.lastLoginIp = ipAddress;
...
// Audit helper: record every login attempt (success or failure)
export async function recordLoginAttempt({ phone, userId = null, ip = null, userAgent = null, success = false, reason = null }) {
  try {
    const LoginAttempt = (await import("./loginAttempt.model.js")).default;
