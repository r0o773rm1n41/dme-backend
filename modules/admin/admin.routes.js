...
router.post("/quiz", roleRequired(["QUIZ_ADMIN", "SUPER_ADMIN"]), async (req, res) => {
  try {
    ...
    if (existingQuiz && ['DRAFT', 'PAYMENT_CLOSED', 'ENDED', 'RESULT_PUBLISHED'].includes(existingQuiz.state)) {
      ...
      fs.createReadStream(req.file.path)
        ...
        .on('end', async () => {
          try {
            ...
            if (existingQuiz && ['DRAFT', 'PAYMENT_CLOSED', 'CREATED', 'CLOSED', 'ENDED', 'FINALIZED', 'RESULT_PUBLISHED'].includes(existingQuiz.state)) {
              await Quiz.deleteOne({ _id: existingQuiz._id });
            }
...
// Admin can snapshot eligible users and close payments manually
router.put("/quiz/:quizDate/close-payments", roleRequired(["QUIZ_ADMIN", "SUPER_ADMIN"]), async (req, res) => {
  try {
    // snapshotEligibleUsers already transitions state to PAYMENT_CLOSED
    const quiz = await QuizService.snapshotEligibleUsers(req.params.quizDate);
    await logAdminAction(req.user._id, 'QUIZ_PAYMENTS_CLOSED', 'QUIZ', req.params.quizDate, { status: 'PAYMENT_CLOSED' }, req);
    res.json(quiz);
  } catch (error) {
    res.status(400).json({ message: error.message });
  }
...
// Disqualify an individual attempt (QUIZ_ADMIN or SUPER_ADMIN)
router.post('/attempts/:attemptId/disqualify', roleRequired(['QUIZ_ADMIN', 'SUPER_ADMIN']), async (req, res) => {
  try {
    const attempt = await QuizAttempt.findById(req.params.attemptId);
    if (!attempt) return res.status(404).json({ message: 'Attempt not found' });

    // mark attempt as not counted
    attempt.counted = false;
    attempt.isEligible = false;
    attempt.eligibilityReason = 'DISQUALIFIED';
    attempt.eligibilitySnapshotAt = new Date();
    await attempt.save();

    await logAdminAction(req.user._id, 'ATTEMPT_DISQUALIFIED', 'QUIZ_ATTEMPT', attempt._id, { quizDate: attempt.quizDate }, req);
    res.json({ success: true, message: 'Attempt disqualified' });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
});
...
// Winners (QUIZ_ADMIN or SUPER_ADMIN) - includes provisional before finalization
router.get("/winners", roleRequired(["QUIZ_ADMIN", "SUPER_ADMIN"]), async (req, res) => {
  try {
    ...
    let winners = await Winner.find({ quizDate })
      .populate('user', 'name phone email profileImage fullName username')
      .sort({ rank: 1 })
      .limit(20);

    // if no winners yet (not finalized) calculate provisional leaderboard
    if (!winners || winners.length === 0) {
      const provisional = await QuizAttempt.find({ quizDate, answersSaved: true, isEligible: true })
        .populate('user', 'name phone email profileImage fullName username')
        .sort({ score: -1, totalTimeMs: 1 })
        .limit(20);
      winners = provisional.map((attempt, idx) => ({
        user: attempt.user,
        rank: idx + 1,
        score: attempt.score,
...
// Get all referrals with status and fraud flags
router.get("/referrals", roleRequired(["SUPER_ADMIN"]), async (req, res) => {
  try {
    ...
    // Count total matching referrals
    const total = await Referral.countDocuments(filter);

    // Fetch referrals with populated user details
    const referrals = await Referral.find(filter)
      .populate('referrer', 'phone email fullName successfulReferrals freeQuizCredits referralCode')
      .populate('referredUser', 'phone email fullName hasPaidBefore hasPdfAccess createdAt')
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(parseInt(limit))
      .lean();

    // Enrich with fraud flags
    const enriched = referrals.map(ref => ({
      ...
      referrerSuccessful: ref.referrer.successfulReferrals,
      referrerRewards: ref.referrer.freeQuizCredits,
      referredUserId: ref.referredUser._id,
      referredPhone: ref.referredUser.phone,
      referredPaid: ref.referredUser.hasPaidBefore,
      referredPdfAccess: ref.referredUser.hasPdfAccess,
      referredAt: ref.referredUser.createdAt,
      status: ref.status,
      completedAt: ref.completedAt,
      deviceIdMatch: ref.deviceIdMatch, // fraud flag
      ipMatch: ref.ipMatch // fraud flag
    }));
