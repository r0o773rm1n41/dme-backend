...
router.post("/", authRequired, writeRateLimit, upload.single("pdf"), validate(blogSchemas.createBlog), async (req, res) => {
  try {
    ...
    if (req.file) {
      const { hasUserPaidEver } = await import("../payment/payment.service.js");
      const hasPaid = await hasUserPaidEver(req.user._id);
      
      if (!hasPaid) {
        return res.status(403).json({ 
          message: "Payment required to upload PDFs. Please make a payment to unlock PDF upload feature. You can still publish text notes without payment." 
        });
      }
...
router.get("/:blogId/pdf", authRequired, async (req, res) => {
  try {
    // Check if user has ever paid (for PDF download access)
    const { hasUserPaidEver } = await import("../payment/payment.service.js");
    const hasPaid = await hasUserPaidEver(req.user._id);
    
    if (!hasPaid) {
      return res.status(403).json({ 
        message: "Payment required to download PDFs. Please make a payment to unlock PDF download feature." 
      });
    }
