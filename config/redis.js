...
if (
  ...
  redisClient = {
    ...
    set: (key, value, options) =>
      upstashClient.set(key, value, options),
    ...
    keys: async (pattern) => {
      console.warn(
        `Redis KEYS not supported by Upstash REST. Pattern: ${pattern}`
      );
      return [];
    },
    ...
    // ---------- TTL ----------
    setEx: (key, ttl, value) =>
      upstashClient.set(key, value, { ex: ttl }),
    ...
    incrby: (key, delta) =>
      upstashClient.incrby(key, delta),

    hincrby: (key, field, increment) =>
      upstashClient.hincrby(key, field, increment),

    hgetall: (key) => upstashClient.hgetall(key),

    // ---------- LISTS (FIXED PROPERLY) ----------
    lpush: (key, ...values) =>
      upstashClient.lpush(key, ...values),

    lrange: (key, start, end) =>
      upstashClient.lrange(key, start, end),

    ltrim: (key, start, stop) =>
      upstashClient.ltrim(key, start, stop),
  };
  ...
  redisClient = {
    ...
    hincrby: (key, field, increment) => {
      ...
      store.set(hashKey, next.toString());
      return Promise.resolve(next);
    },
  ...
  console.log("🧪 Using mock Redis (local mode)");
}
